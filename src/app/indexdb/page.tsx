...
export default function IndexDBPage() {
    ...
    useEffect(() => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            //buat object store
            if (!databse.objectStoreNames.contains("todos")) {
                const objectStore = databse.createObjectStore("todos", { keyPath: "id", autoIncrement: true});
    ...
    const toggleTodo = (id: number) => {
        if (!db) return;
        const transaction = db.transaction(["todos"], "readwrite");
        const obbjectStore = transaction.objectStore("todos");
        const request = obbjectStore.get(id);

        request.onsuccess = (event) => {
            const todo = (event.target as IDBRequest).result;
            todo.completed = !todo.completed;

            const updateRequest = obbjectStore.put(todo);
